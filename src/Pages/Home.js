...
export const Home = (props) => {
  ...
  const handleSubmit = (e) => {
    if (options.length < 2) toast.error("Minimum 2 options required!");
    else {
      let flag = 0;
      ...
      if (flag === 0) {
        if (title === "") {
          ...
          createPoll(poll);
          toast.success(
            "Poll Generated Successfully 🎉 Share the link to your friends and family"
          );
          setTimeout(() => {
  ...
  const handleChange = (index, e) => {
    let x = options;
    x.forEach((option) => {
      if (option.index === index) option.title = e.target.value;
    });
    setOptions([...x]);
  };
